...
#' @param states.ini initial parameters
...
sacsma <- function(par, states.ini = c(0, 0, 5, 5, 5, 0), prcp, pet, lat, elev, 
  ...
  uzfwc <- states.ini[2]  # Upper zone free water storage
  lztwc <- states.ini[3]  # Lower zone tension water storage
  lzfsc <- states.ini[4]  # Lower zone supplementary free water storage
  lzfpc <- states.ini[5]  # Upper zone primary free water storage
  adimc <- states.ini[6]  # Additional impervious area storage
  ...
  thres_zero <- 1e-05  # Threshold to be considered as zero
  parea <- 1 - adimp - pctim
  ...
  for (i in 1:length(prcp)) {
    ...
    if (uztwc <= 0) {
      ...
      # in case et1 <= uztws, all maximum et (et1) are consumed at uztwc, so no et from uzfwc (et2=0)
    } else {
      
      # There's possibility that upper zone free water ratio exceeds upper zone tension water ratio. If
      # so, free water is transferred to tension water storage
    ...
    # ET(5), ET from additional impervious (ADIMP) area ????? no idea where this come from, I think
    # there's a possibility that et5 can be negative values
    ...
    for (n in 1:ninc) {
      ...
      # Compute baseflow and keep track of time interval sum Baseflow from free water primary storage
      bf_p <- lzfpc * dlzp
      lzfpc <- lzfpc - bf_p
      if (lzfpc <= 1e-04) {
        bf_p <- bf_p + lzfpc
      ...
      if ((pinc + uzfwc) <= 0.01) {
        ...
        # the PFREE area. PERCT is percolation to tension water and PERCF is percolation going to free
        # water.
