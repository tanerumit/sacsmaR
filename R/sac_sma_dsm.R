...
hru_area <- gridinfo[3]
hru_elev <- gridinfo[4]
hru_flowlen <- gridinfo[5]

str_date = as.Date("1995/10/1")
end_date = as.Date("2014/09/30")

#Calibrated parameters
hru_par  = read.table(paste0(parent_filedir,'HRUfiles/hru_optpar_0425_arrhon_KGE75.txt'))
...
sim_per  <- datenum(end_date)-datenum(str_date)+1; # Simulation period
...
for (n in 1:num_hru) {
  ...
  #Model parameters
  pars_sac <- hru_par[n,1:27] %>% as.numeric() # SAC_SMA parameters for a HRU
  
  #Simulate streamflow from SAC-SMA model
  hru_simflow <- sac_sma(str_date, end_date, hru_lat, hru_lon, hru_area, hru_elev, hru_flowlen, hru_par) 
